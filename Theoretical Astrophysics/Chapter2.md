...
#### 碱金属原子（原子实 + 价层电子）
##### 原子实
对于多电子原子体系，为什么不能所有的电子都处在同一个最低能量的态上呢？考虑到体系总是趋向于能量最低体系。泡利提出了“不相容原理”，表示同一个态不能存在两个或以上的电子（费米子）；也就是因为如此，电子出现壳层结构。

是由原子核和满壳层的电子组成的体系，其**总轨道角动量**和**总自旋角动量**都是零，但原子实外的价电子所受的力不再是简单的库仑力**，要考虑**原子实的极化**以及**电子轨道在原子实中的穿透。
...
# 原子的激发和电离
### 激发
1. 自发辐射：当原子停留在某一非基态的定态时,处于高能态（激发电势 $\varepsilon_k$ ）原子会自发地跃迁到低能态（ $\varepsilon_i$）而同时辐射出光量子（频率 $\nu_{ik}=\frac{\epsilon_k-\epsilon_i}{h}$ ）
2. 光致激发/激发过程：当原子处于辐射场中时,处于低能态的原子如果吸收了一个能量刚好和高、低态能量之差对应的光量子,原子就会从低能态跃迁到高能态上去
3. 激发能：把原子从基态激发到某一能态所需要施给的能量
4. 碰撞激发：通过非弹性碰撞激发，部分动能转化为内能（第一类碰撞，低能原子与其他粒子碰撞的时候，将动能转换成使原子激发的内能）
5. 碰撞降激发: 即第二类碰撞，高能原子将激发能转给与之碰撞的粒子而不发出辐射，可看作第一类碰撞的的逆过程
### 电离（束缚$\to$自由跃迁）
