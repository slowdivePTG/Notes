...
#### 自发发射概率系数 $A_{ki}$
在没有外场的情况下,单位时间一个原子从高能级 $k$ 跃迁到低能级 $i$ 同时在 $\text{d}\omega$ 立体角内发射一个能量为 $h\nu_{ki}$ 的光量子的概率为 $A _ { k i } \text{d} \omega / 4 \pi$
#### 受激发射概率系数 $B_{ki}$
在强度为 $I_\nu$ 的**辐射场作用下**,单位时间一个原子从高能级 $k$ 跃迁到低能级 $i$ 同时在 $\text{d}\omega$ 立体角内发射一个能量为 $h\nu_{ki}$ 的光量子的概率为 $B _ { k i } I_\nu\text{d} \omega / 4 \pi$
#### 受激吸收概率系数 $B_{ik}$
受激发射的逆过程
