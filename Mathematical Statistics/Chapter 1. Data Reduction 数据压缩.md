...
- 例：泊松分布：$P \left( X _ { i } = x \right) = e ^ { - \theta } ( \theta ) ^ { x } / x !$
  ...
  - $n$ 个iid的泊松分布之和 $\sim Poisson(n\theta)$
    $$
    q(\sum x_i|\theta)=e ^ { - n\theta } ( n\theta ) ^ { \sum x_i } / \left(\sum x_i\right) !
    $$
...
- 指数分布族：
  $$
  f ( x | \theta ) = h ( x ) c ( \theta ) \exp \left( \sum _ { j = 1 } ^ { k } w \left( \theta _ { j } \right) t _ { j } ( x ) \right)
  $$

- $X_1,\cdots,X_n$ 是iid的观测结果

- 统计量：
  $$
  T ( X ) = \left( \sum _ { i = 1 } ^ { k } t _ { 1 } \left( X _ { i } \right) , \ldots , \sum _ { i = 1 } ^ { k } t _ { k } \left( X _ { i } \right) \right)
  $$
  是完备的，只要参数空间包含 $\mathbb{R}^k$ 中的一个开集
